...
// These fields are auto-generated by the system and must never be accepted from user input.
// Auth-collection fields like email/password are also x-system but ARE user-provided.
...
function rejectSystemFields(
  ...
  const sysKeys = Object.keys(data).filter((k) => GENERATED_FIELDS.has(k))
